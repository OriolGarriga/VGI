...
/*
// Eix X (vermell) Moviment tecles A i D
...
// Eix Z (blau) Moviment tecles W i S
...
//Adaptacio de la funcio vista_navega per tal que funcioni de la manera que volem nosaltres: Moviment de la camera amb WASD i rotacio de la mateixa utilitzant el ratoli
glm::mat4 nostra_Vista(GLuint sh_programID, glm::vec3 position,
	CColor col_fons, CColor col_object, char objecte, double mida, int step,
	bool frnt_fcs, bool oculta, bool testv, bool bck_ln,
	char iluminacio, bool llum_amb, LLUM* lumi, bool ifix, bool il2sides,
	bool eix, CMask3D reixa, CPunt3D hreixa, float angle_h, float angle_v)
{
	...
	3 paràmetres:
		- eye: Lloc on esta la camara
		- center: Lloc on apunta la camara
		- up: Eix que ens queda per sobre el cap 
	*/
	glm::mat4 MVista = glm::lookAt(position, position + direccio_cam, up);

	/*La funció gluniformrmatrix4fv carrega una matriu de 4x4 al shader actual
	4 paràmetres:
		-glGetUniformLocation(sh_programID, "viewMatrix"): Obté la ubicació de la variable viewMatrix en el shader que té la ID sh_programID.
		Aixi el opengl sap on carregar la matriu
		- 1: Nº de matrius que estem carregant simultaneament (Millor de 1 en 1)
		- GL_FALSE: Indica si hem de transposar la matriu abans de fer res
		- &MVista[0][0]: Apuntador al primer element de la matriu que volem carregar
	*/
	glUniformMatrix4fv(glGetUniformLocation(sh_programID, "viewMatrix"), 1, GL_FALSE, &MVista[0][0]);
