...
// Entorn VGI: Definició de la càmera.
		...
		else if (camera == NOSTRA_CAM) {
			ViewMatrix = nostra_Vista(shader_programID, Camara::M_C.posi,
...
// (1) ALWAYS forward mouse data to ImGui! This is automatic with default backends. With your own backend:
	...
	if (!io.WantCaptureKeyboard) { //<Tractament mouse de l'aplicació>}
		...
		//Si nomes apretem la tecla N:
		//Fem inservible el cursor per tal de facilitar el moviment i fem que desaparegui, canviem el mode de càmera i settejem la posició on som
		...
		//Si la camara es la nostra i s'ha apretat qualsevol lletra:
		else if (camera == NOSTRA_CAM && action == GLFW_PRESS) {
			if (key == GLFW_KEY_W) w_p = true; //Si s'ha apretat w, ho fiquem a true
			...
			if (key == GLFW_KEY_S) s_p = false; //Si no s'ha apretat s, ho fiquem a false
			if (key == GLFW_KEY_A) a_p = false; //Si no s'ha apretat a, ho fiquem a false
			if (key == GLFW_KEY_D) d_p = false; //Si no s'ha apretat d, ho fiquem a false
		}
